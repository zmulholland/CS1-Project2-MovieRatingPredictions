...
# Function that computes the similarity between two users
def similarity(u, v, rLu):
    # Create a list of movies that both users have rated
    C = []
    for movie in rLu[u - 1]:
        if movie in rLu[v - 1]:
            C.append(movie)
    if C == []:
        return 0
    uMean = meanUserRatingPrediction(u, 0, rLu)
    vMean = meanUserRatingPrediction(v, 0, rLu)
    
    numerator = 0
    denominator1 = 0
    denominator2 = 0
    for m in C:
        numerator = numerator + (rLu[u-1][m] - uMean) * (rLu[v-1][m] - vMean)
        denominator1 = denominator1 + (rLu[u-1][m] - uMean) ** 2
        denominator2 = denominator2 + (rLu[v-1][m] - vMean) ** 2
...
# Function that returns a list of (user, similarity) for the k most similar users to u
def kNearestNeighbors(u, rLu, k):
    neighborList = []

    for i in range(len(rLu)):
        if i + 1 != u:      
            neighborList.append((i + 1, similarity(u, i + 1, rLu)))
    mid = time.time()
    neighborList.sort(reverse=True, key=lambda x: x[1])
    return neighborList[:k]

# Calculates the weighted average of similar users for a prediction
def CFRatingPrediction(u, m, rLu, friends):
    uMean = meanUserRatingPrediction(u, 0, rLu)
    U = []
    for neighbor in friends:
        if m in rLu[neighbor[0] - 1]:
            U.append(neighbor)
    numerator = 0
    denominator = 0
    ...
    for j in U:
        similarityNum = j[1]
        numerator = numerator + (rLu[j[0]-1][m] - meanUserRatingPrediction(j[0], 0, rLu)) * similarityNum
        denominator = denominator + abs(similarityNum)
        
    if denominator == 0:
        return uMean

    return uMean + numerator / denominator
...
for i in range(10):
    ...
    friends = []
    for i in range(len(userList)):
        friends.append(kNearestNeighbors(i + 1, trainingRLu, len(userList)))
    stop = time.time()
    print("Neighbors", stop-start)
    ...
    for rating in testSet:
        ...
        cf10.append(CFRatingPrediction(rating[0], rating[1], trainingRLu, friends[rating[0]-1][:10]))
        cf100.append(CFRatingPrediction(rating[0], rating[1], trainingRLu, friends[rating[0]-1][:100]))
        cf500.append(CFRatingPrediction(rating[0], rating[1], trainingRLu, friends[rating[0]-1][:500]))
        cfAll.append(CFRatingPrediction(rating[0], rating[1], trainingRLu, friends[rating[0]-1]))
    
    
    stop = time.time()
    print("Appending", stop-start)
    ...
    ave10.append(rmse(actualRatings, cf10))
    ave100.append(rmse(actualRatings, cf100))
    ave500.append(rmse(actualRatings, cf500))
    aveAll.append(rmse(actualRatings, cfAll))


data = [randAve, userAve, movieAve, demAve, genreAve, ave10, ave100, ave500, aveAll]
labels = ["Algo1", "Algo2", "Algo3", "Algo4", "Algo5", "Algo6", "Algo7", "Algo8", "Algo9"]
draw_boxplot(data, labels)

data = [userAve, movieAve, demAve, genreAve, ave10, ave100, ave500, aveAll]
labels = ["Algo2", "Algo3", "Algo4", "Algo5", "Algo6", "Algo7", "Algo8", "Algo9"]
draw_boxplot(data, labels)
